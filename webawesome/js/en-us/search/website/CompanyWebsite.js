...
async function websocketCompanyWebsite(success) {
  window.eventBus.onopen = function () {
    ...
    window.eventBus.registerHandler('websocketCompanyWebsite', function (error, message) {
      ...
      iTemplate.innerHTML = '<i class="fa-duotone fa-regular fa-globe-pointer"></i>';
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        if(vars.includes('created'))
          inputCreated = $response.querySelector('.CompanyWebsite_Page_created');
        if(vars.includes('modified'))
          inputModified = $response.querySelector('.CompanyWebsite_Page_modified');
        if(vars.includes('archived'))
          inputArchived = $response.querySelector('.CompanyWebsite_Page_archived');
        if(vars.includes('name'))
          inputName = $response.querySelector('.CompanyWebsite_Page_name');
        if(vars.includes('description'))
          inputDescription = $response.querySelector('.CompanyWebsite_Page_description');
        if(vars.includes('pageId'))
          inputPageId = $response.querySelector('.CompanyWebsite_Page_pageId');
        if(vars.includes('displayPage'))
          inputDisplayPage = $response.querySelector('.CompanyWebsite_Page_displayPage');
        if(vars.includes('classCanonicalName'))
          inputClassCanonicalName = $response.querySelector('.CompanyWebsite_Page_classCanonicalName');
        if(vars.includes('classSimpleName'))
          inputClassSimpleName = $response.querySelector('.CompanyWebsite_Page_classSimpleName');
        if(vars.includes('classCanonicalNames'))
          inputClassCanonicalNames = $response.querySelector('.CompanyWebsite_Page_classCanonicalNames');
        if(vars.includes('saves'))
          inputSaves = $response.querySelector('.CompanyWebsite_Page_saves');
        if(vars.includes('objectTitle'))
          inputObjectTitle = $response.querySelector('.CompanyWebsite_Page_objectTitle');
        if(vars.includes('editPage'))
          inputEditPage = $response.querySelector('.CompanyWebsite_Page_editPage');
        if(vars.includes('userPage'))
          inputUserPage = $response.querySelector('.CompanyWebsite_Page_userPage');
        if(vars.includes('download'))
          inputDownload = $response.querySelector('.CompanyWebsite_Page_download');
        if(vars.includes('objectSuggest'))
          inputObjectSuggest = $response.querySelector('.CompanyWebsite_Page_objectSuggest');
        if(vars.includes('objectText'))
          inputObjectText = $response.querySelector('.CompanyWebsite_Page_objectText');
        if(vars.includes('solrId'))
          inputSolrId = $response.querySelector('.CompanyWebsite_Page_solrId');
        if(vars.includes('websiteNum'))
          inputWebsiteNum = $response.querySelector('.CompanyWebsite_Page_websiteNum');

        jsWebsocketCompanyWebsite(pageId, vars, $response);
        window.result = JSON.parse($response.querySelector('.pageForm .result')?.value);
        window.listCompanyWebsite = JSON.parse($response.querySelector('.pageForm .listCompanyWebsite')?.value);


        if(inputCreated) {
          document.querySelectorAll('.CompanyWebsite_Page_created').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputCreated.getAttribute('value');
            else
              item.textContent = inputCreated.textContent;
          });
          addGlow(document.querySelector('.CompanyWebsite_Page_created'));
        }

        if(inputModified) {
          document.querySelectorAll('.CompanyWebsite_Page_modified').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputModified.getAttribute('value');
            else
              item.textContent = inputModified.textContent;
          });
          addGlow(document.querySelector('.CompanyWebsite_Page_modified'));
        }

        if(inputArchived) {
          document.querySelectorAll('.CompanyWebsite_Page_archived').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputArchived.getAttribute('value');
            else
              item.textContent = inputArchived.textContent;
          });
          addGlow(document.querySelector('.CompanyWebsite_Page_archived'));
        }

        if(inputName) {
          document.querySelectorAll('.CompanyWebsite_Page_name').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputName.getAttribute('value');
            else
              item.textContent = inputName.textContent;
          });
          addGlow(document.querySelector('.CompanyWebsite_Page_name'));
        }

        if(inputDescription) {
          document.querySelectorAll('.CompanyWebsite_Page_description').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDescription.getAttribute('value');
            else
              item.textContent = inputDescription.textContent;
          });
          addGlow(document.querySelector('.CompanyWebsite_Page_description'));
        }

        if(inputPageId) {
          document.querySelectorAll('.CompanyWebsite_Page_pageId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageId.getAttribute('value');
            else
              item.textContent = inputPageId.textContent;
          });
          addGlow(document.querySelector('.CompanyWebsite_Page_pageId'));
        }

        if(inputDisplayPage) {
          document.querySelectorAll('.CompanyWebsite_Page_displayPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDisplayPage.getAttribute('value');
            else
              item.textContent = inputDisplayPage.textContent;
          });
          addGlow(document.querySelector('.CompanyWebsite_Page_displayPage'));
        }

        if(inputClassCanonicalName) {
          document.querySelectorAll('.CompanyWebsite_Page_classCanonicalName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassCanonicalName.getAttribute('value');
            else
              item.textContent = inputClassCanonicalName.textContent;
          });
          addGlow(document.querySelector('.CompanyWebsite_Page_classCanonicalName'));
        }

        if(inputClassSimpleName) {
          document.querySelectorAll('.CompanyWebsite_Page_classSimpleName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassSimpleName.getAttribute('value');
            else
              item.textContent = inputClassSimpleName.textContent;
          });
          addGlow(document.querySelector('.CompanyWebsite_Page_classSimpleName'));
        }

        if(inputClassCanonicalNames) {
          document.querySelectorAll('.CompanyWebsite_Page_classCanonicalNames').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassCanonicalNames.getAttribute('value');
            else
              item.textContent = inputClassCanonicalNames.textContent;
          });
          addGlow(document.querySelector('.CompanyWebsite_Page_classCanonicalNames'));
        }

        if(inputSaves) {
          document.querySelectorAll('.CompanyWebsite_Page_saves').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSaves.getAttribute('value');
            else
              item.textContent = inputSaves.textContent;
          });
          addGlow(document.querySelector('.CompanyWebsite_Page_saves'));
        }

        if(inputObjectTitle) {
          document.querySelectorAll('.CompanyWebsite_Page_objectTitle').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectTitle.getAttribute('value');
            else
              item.textContent = inputObjectTitle.textContent;
          });
          addGlow(document.querySelector('.CompanyWebsite_Page_objectTitle'));
        }

        if(inputEditPage) {
          document.querySelectorAll('.CompanyWebsite_Page_editPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputEditPage.getAttribute('value');
            else
              item.textContent = inputEditPage.textContent;
          });
          addGlow(document.querySelector('.CompanyWebsite_Page_editPage'));
        }

        if(inputUserPage) {
          document.querySelectorAll('.CompanyWebsite_Page_userPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputUserPage.getAttribute('value');
            else
              item.textContent = inputUserPage.textContent;
          });
          addGlow(document.querySelector('.CompanyWebsite_Page_userPage'));
        }

        if(inputDownload) {
          document.querySelectorAll('.CompanyWebsite_Page_download').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDownload.getAttribute('value');
            else
              item.textContent = inputDownload.textContent;
          });
          addGlow(document.querySelector('.CompanyWebsite_Page_download'));
        }

        if(inputObjectSuggest) {
          document.querySelectorAll('.CompanyWebsite_Page_objectSuggest').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectSuggest.getAttribute('value');
            else
              item.textContent = inputObjectSuggest.textContent;
          });
          addGlow(document.querySelector('.CompanyWebsite_Page_objectSuggest'));
        }

        if(inputObjectText) {
          document.querySelectorAll('.CompanyWebsite_Page_objectText').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectText.getAttribute('value');
            else
              item.textContent = inputObjectText.textContent;
          });
          addGlow(document.querySelector('.CompanyWebsite_Page_objectText'));
        }

        if(inputSolrId) {
          document.querySelectorAll('.CompanyWebsite_Page_solrId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSolrId.getAttribute('value');
            else
              item.textContent = inputSolrId.textContent;
          });
          addGlow(document.querySelector('.CompanyWebsite_Page_solrId'));
        }

        if(inputWebsiteNum) {
          document.querySelectorAll('.CompanyWebsite_Page_websiteNum').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputWebsiteNum.getAttribute('value');
            else
              item.textContent = inputWebsiteNum.textContent;
          });
          addGlow(document.querySelector('.CompanyWebsite_Page_websiteNum'));
        }
...
function searchCompanyWebsiteVals(filters, target, success, error) {
  ...
  fetch(
    '/en-us/api/website?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
  ...
  success = function( data, textStatus, jQxhr ) {
    if($list) {
      ...
      data['list'].forEach((o, i) => {
        var $i = document.querySelector('<i class="fa-duotone fa-regular fa-globe-pointer"></i>');
...
async function patchCompanyWebsite($formFilters, $formValues, target, pageId, success, error) {
  ...
  patchCompanyWebsiteVals(pageId == null ? deparam(window.location.search ? window.location.search.substring(1) : window.location.search) : [{name:'fq', value:'pageId:' + pageId}], vals, target, success, error);
}
...
async function putimportCompanyWebsite($formValues, target, pageId, success, error) {
