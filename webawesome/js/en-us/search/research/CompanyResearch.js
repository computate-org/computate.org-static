...
async function websocketCompanyResearch(success) {
  window.eventBus.onopen = function () {
    ...
    window.eventBus.registerHandler('websocketCompanyResearch', function (error, message) {
      ...
      iTemplate.innerHTML = '<i class="fa-duotone fa-regular fa-school"></i>';
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        if(vars.includes('created'))
          inputCreated = $response.querySelector('.CompanyResearch_Page_created');
        if(vars.includes('modified'))
          inputModified = $response.querySelector('.CompanyResearch_Page_modified');
        if(vars.includes('archived'))
          inputArchived = $response.querySelector('.CompanyResearch_Page_archived');
        if(vars.includes('name'))
          inputName = $response.querySelector('.CompanyResearch_Page_name');
        if(vars.includes('description'))
          inputDescription = $response.querySelector('.CompanyResearch_Page_description');
        if(vars.includes('pageId'))
          inputPageId = $response.querySelector('.CompanyResearch_Page_pageId');
        if(vars.includes('displayPage'))
          inputDisplayPage = $response.querySelector('.CompanyResearch_Page_displayPage');
        if(vars.includes('classCanonicalName'))
          inputClassCanonicalName = $response.querySelector('.CompanyResearch_Page_classCanonicalName');
        if(vars.includes('classSimpleName'))
          inputClassSimpleName = $response.querySelector('.CompanyResearch_Page_classSimpleName');
        if(vars.includes('classCanonicalNames'))
          inputClassCanonicalNames = $response.querySelector('.CompanyResearch_Page_classCanonicalNames');
        if(vars.includes('saves'))
          inputSaves = $response.querySelector('.CompanyResearch_Page_saves');
        if(vars.includes('objectTitle'))
          inputObjectTitle = $response.querySelector('.CompanyResearch_Page_objectTitle');
        if(vars.includes('editPage'))
          inputEditPage = $response.querySelector('.CompanyResearch_Page_editPage');
        if(vars.includes('userPage'))
          inputUserPage = $response.querySelector('.CompanyResearch_Page_userPage');
        if(vars.includes('download'))
          inputDownload = $response.querySelector('.CompanyResearch_Page_download');
        if(vars.includes('objectSuggest'))
          inputObjectSuggest = $response.querySelector('.CompanyResearch_Page_objectSuggest');
        if(vars.includes('objectText'))
          inputObjectText = $response.querySelector('.CompanyResearch_Page_objectText');
        if(vars.includes('solrId'))
          inputSolrId = $response.querySelector('.CompanyResearch_Page_solrId');

        jsWebsocketCompanyResearch(pageId, vars, $response);
        window.result = JSON.parse($response.querySelector('.pageForm .result')?.value);
        window.listCompanyResearch = JSON.parse($response.querySelector('.pageForm .listCompanyResearch')?.value);


        if(inputCreated) {
          document.querySelectorAll('.CompanyResearch_Page_created').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputCreated.getAttribute('value');
            else
              item.textContent = inputCreated.textContent;
          });
          addGlow(document.querySelector('.CompanyResearch_Page_created'));
        }

        if(inputModified) {
          document.querySelectorAll('.CompanyResearch_Page_modified').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputModified.getAttribute('value');
            else
              item.textContent = inputModified.textContent;
          });
          addGlow(document.querySelector('.CompanyResearch_Page_modified'));
        }

        if(inputArchived) {
          document.querySelectorAll('.CompanyResearch_Page_archived').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputArchived.getAttribute('value');
            else
              item.textContent = inputArchived.textContent;
          });
          addGlow(document.querySelector('.CompanyResearch_Page_archived'));
        }

        if(inputName) {
          document.querySelectorAll('.CompanyResearch_Page_name').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputName.getAttribute('value');
            else
              item.textContent = inputName.textContent;
          });
          addGlow(document.querySelector('.CompanyResearch_Page_name'));
        }

        if(inputDescription) {
          document.querySelectorAll('.CompanyResearch_Page_description').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDescription.getAttribute('value');
            else
              item.textContent = inputDescription.textContent;
          });
          addGlow(document.querySelector('.CompanyResearch_Page_description'));
        }

        if(inputPageId) {
          document.querySelectorAll('.CompanyResearch_Page_pageId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageId.getAttribute('value');
            else
              item.textContent = inputPageId.textContent;
          });
          addGlow(document.querySelector('.CompanyResearch_Page_pageId'));
        }

        if(inputDisplayPage) {
          document.querySelectorAll('.CompanyResearch_Page_displayPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDisplayPage.getAttribute('value');
            else
              item.textContent = inputDisplayPage.textContent;
          });
          addGlow(document.querySelector('.CompanyResearch_Page_displayPage'));
        }

        if(inputClassCanonicalName) {
          document.querySelectorAll('.CompanyResearch_Page_classCanonicalName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassCanonicalName.getAttribute('value');
            else
              item.textContent = inputClassCanonicalName.textContent;
          });
          addGlow(document.querySelector('.CompanyResearch_Page_classCanonicalName'));
        }

        if(inputClassSimpleName) {
          document.querySelectorAll('.CompanyResearch_Page_classSimpleName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassSimpleName.getAttribute('value');
            else
              item.textContent = inputClassSimpleName.textContent;
          });
          addGlow(document.querySelector('.CompanyResearch_Page_classSimpleName'));
        }

        if(inputClassCanonicalNames) {
          document.querySelectorAll('.CompanyResearch_Page_classCanonicalNames').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassCanonicalNames.getAttribute('value');
            else
              item.textContent = inputClassCanonicalNames.textContent;
          });
          addGlow(document.querySelector('.CompanyResearch_Page_classCanonicalNames'));
        }

        if(inputSaves) {
          document.querySelectorAll('.CompanyResearch_Page_saves').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSaves.getAttribute('value');
            else
              item.textContent = inputSaves.textContent;
          });
          addGlow(document.querySelector('.CompanyResearch_Page_saves'));
        }

        if(inputObjectTitle) {
          document.querySelectorAll('.CompanyResearch_Page_objectTitle').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectTitle.getAttribute('value');
            else
              item.textContent = inputObjectTitle.textContent;
          });
          addGlow(document.querySelector('.CompanyResearch_Page_objectTitle'));
        }

        if(inputEditPage) {
          document.querySelectorAll('.CompanyResearch_Page_editPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputEditPage.getAttribute('value');
            else
              item.textContent = inputEditPage.textContent;
          });
          addGlow(document.querySelector('.CompanyResearch_Page_editPage'));
        }

        if(inputUserPage) {
          document.querySelectorAll('.CompanyResearch_Page_userPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputUserPage.getAttribute('value');
            else
              item.textContent = inputUserPage.textContent;
          });
          addGlow(document.querySelector('.CompanyResearch_Page_userPage'));
        }

        if(inputDownload) {
          document.querySelectorAll('.CompanyResearch_Page_download').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDownload.getAttribute('value');
            else
              item.textContent = inputDownload.textContent;
          });
          addGlow(document.querySelector('.CompanyResearch_Page_download'));
        }

        if(inputObjectSuggest) {
          document.querySelectorAll('.CompanyResearch_Page_objectSuggest').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectSuggest.getAttribute('value');
            else
              item.textContent = inputObjectSuggest.textContent;
          });
          addGlow(document.querySelector('.CompanyResearch_Page_objectSuggest'));
        }

        if(inputObjectText) {
          document.querySelectorAll('.CompanyResearch_Page_objectText').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectText.getAttribute('value');
            else
              item.textContent = inputObjectText.textContent;
          });
          addGlow(document.querySelector('.CompanyResearch_Page_objectText'));
        }

        if(inputSolrId) {
          document.querySelectorAll('.CompanyResearch_Page_solrId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSolrId.getAttribute('value');
            else
              item.textContent = inputSolrId.textContent;
          });
          addGlow(document.querySelector('.CompanyResearch_Page_solrId'));
        }
    ...
    if(json['facetCounts']) {
      ...
      if(facetCounts['facetPivot'] && facetCounts['facetRanges']) {
        ...
        if(range) {
          ...
          Plotly.react('htmBodyGraphCompanyResearchPage', data, layout);
        }
      }
    }
  }
...
function searchCompanyResearchVals(filters, target, success, error) {
  ...
  fetch(
    '/en-us/api/research?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
  ...
  success = function( data, textStatus, jQxhr ) {
    if($list) {
      ...
      data['list'].forEach((o, i) => {
        var $i = document.querySelector('<i class="fa-duotone fa-regular fa-school"></i>');
...
async function patchCompanyResearch($formFilters, $formValues, target, pageId, success, error) {
  ...
  patchCompanyResearchVals(pageId == null ? deparam(window.location.search ? window.location.search.substring(1) : window.location.search) : [{name:'fq', value:'pageId:' + pageId}], vals, target, success, error);
}
...
async function putimportCompanyResearch($formValues, target, pageId, success, error) {
