...
async function websocketLearnSkills(success) {
  window.eventBus.onopen = function () {

    window.eventBus.registerHandler('websocketLearnSkills', function (error, message) {
      ...
      $headerSpan.innerText = 'modify Learn cloud developer skills in ' + json.timeRemaining;
...
async function websocketLearnSkillsInner(apiRequest) {
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        if(vars.includes('created'))
          inputCreated = $response.querySelector('.LearnSkills_Page_created');
        if(vars.includes('modified'))
          inputModified = $response.querySelector('.LearnSkills_Page_modified');
        if(vars.includes('archived'))
          inputArchived = $response.querySelector('.LearnSkills_Page_archived');
        if(vars.includes('name'))
          inputName = $response.querySelector('.LearnSkills_Page_name');
        if(vars.includes('description'))
          inputDescription = $response.querySelector('.LearnSkills_Page_description');
        if(vars.includes('authorName'))
          inputAuthorName = $response.querySelector('.LearnSkills_Page_authorName');
        if(vars.includes('authorUrl'))
          inputAuthorUrl = $response.querySelector('.LearnSkills_Page_authorUrl');
        if(vars.includes('pageImageUri'))
          inputPageImageUri = $response.querySelector('.LearnSkills_Page_pageImageUri');
        if(vars.includes('pageId'))
          inputPageId = $response.querySelector('.LearnSkills_Page_pageId');
        if(vars.includes('displayPage'))
          inputDisplayPage = $response.querySelector('.LearnSkills_Page_displayPage');
        if(vars.includes('classCanonicalName'))
          inputClassCanonicalName = $response.querySelector('.LearnSkills_Page_classCanonicalName');
        if(vars.includes('classSimpleName'))
          inputClassSimpleName = $response.querySelector('.LearnSkills_Page_classSimpleName');
        if(vars.includes('classCanonicalNames'))
          inputClassCanonicalNames = $response.querySelector('.LearnSkills_Page_classCanonicalNames');
        if(vars.includes('saves'))
          inputSaves = $response.querySelector('.LearnSkills_Page_saves');
        if(vars.includes('objectTitle'))
          inputObjectTitle = $response.querySelector('.LearnSkills_Page_objectTitle');
        if(vars.includes('editPage'))
          inputEditPage = $response.querySelector('.LearnSkills_Page_editPage');
        if(vars.includes('userPage'))
          inputUserPage = $response.querySelector('.LearnSkills_Page_userPage');
        if(vars.includes('download'))
          inputDownload = $response.querySelector('.LearnSkills_Page_download');
        if(vars.includes('objectSuggest'))
          inputObjectSuggest = $response.querySelector('.LearnSkills_Page_objectSuggest');
        if(vars.includes('objectText'))
          inputObjectText = $response.querySelector('.LearnSkills_Page_objectText');
        if(vars.includes('solrId'))
          inputSolrId = $response.querySelector('.LearnSkills_Page_solrId');
        if(vars.includes('courseNum'))
          inputCourseNum = $response.querySelector('.LearnSkills_Page_courseNum');
        if(vars.includes('pageVideoUrl'))
          inputPageVideoUrl = $response.querySelector('.LearnSkills_Page_pageVideoUrl');
        if(vars.includes('pageImageWidth'))
          inputPageImageWidth = $response.querySelector('.LearnSkills_Page_pageImageWidth');
        if(vars.includes('pageImageHeight'))
          inputPageImageHeight = $response.querySelector('.LearnSkills_Page_pageImageHeight');
        if(vars.includes('pageImageType'))
          inputPageImageType = $response.querySelector('.LearnSkills_Page_pageImageType');
        if(vars.includes('pageImageAlt'))
          inputPageImageAlt = $response.querySelector('.LearnSkills_Page_pageImageAlt');
        if(vars.includes('prerequisiteArticleIds'))
          inputPrerequisiteArticleIds = $response.querySelector('.LearnSkills_Page_prerequisiteArticleIds');
        if(vars.includes('prerequisiteArticles'))
          inputPrerequisiteArticles = $response.querySelector('.LearnSkills_Page_prerequisiteArticles');
        if(vars.includes('nextArticleIds'))
          inputNextArticleIds = $response.querySelector('.LearnSkills_Page_nextArticleIds');
        if(vars.includes('nextArticles'))
          inputNextArticles = $response.querySelector('.LearnSkills_Page_nextArticles');
        if(vars.includes('labelsString'))
          inputLabelsString = $response.querySelector('.LearnSkills_Page_labelsString');
        if(vars.includes('labels'))
          inputLabels = $response.querySelector('.LearnSkills_Page_labels');
        if(vars.includes('relatedArticleIds'))
          inputRelatedArticleIds = $response.querySelector('.LearnSkills_Page_relatedArticleIds');

        jsWebsocketLearnSkills(pageId, vars, $response);
        window.result = JSON.parse($response.querySelector('.pageForm .result')?.value);
        window.listLearnSkills = JSON.parse($response.querySelector('.pageForm .listLearnSkills')?.value);


        if(inputCreated) {
          document.querySelectorAll('.LearnSkills_Page_created').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputCreated.getAttribute('value');
            else
              item.textContent = inputCreated.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_created'));
        }

        if(inputModified) {
          document.querySelectorAll('.LearnSkills_Page_modified').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputModified.getAttribute('value');
            else
              item.textContent = inputModified.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_modified'));
        }

        if(inputArchived) {
          document.querySelectorAll('.LearnSkills_Page_archived').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputArchived.getAttribute('value');
            else
              item.textContent = inputArchived.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_archived'));
        }

        if(inputName) {
          document.querySelectorAll('.LearnSkills_Page_name').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputName.getAttribute('value');
            else
              item.textContent = inputName.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_name'));
        }

        if(inputDescription) {
          document.querySelectorAll('.LearnSkills_Page_description').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDescription.getAttribute('value');
            else
              item.textContent = inputDescription.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_description'));
        }

        if(inputAuthorName) {
          document.querySelectorAll('.LearnSkills_Page_authorName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAuthorName.getAttribute('value');
            else
              item.textContent = inputAuthorName.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_authorName'));
        }

        if(inputAuthorUrl) {
          document.querySelectorAll('.LearnSkills_Page_authorUrl').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAuthorUrl.getAttribute('value');
            else
              item.textContent = inputAuthorUrl.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_authorUrl'));
        }

        if(inputPageImageUri) {
          document.querySelectorAll('.LearnSkills_Page_pageImageUri').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageImageUri.getAttribute('value');
            else
              item.textContent = inputPageImageUri.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_pageImageUri'));
        }

        if(inputPageId) {
          document.querySelectorAll('.LearnSkills_Page_pageId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageId.getAttribute('value');
            else
              item.textContent = inputPageId.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_pageId'));
        }

        if(inputDisplayPage) {
          document.querySelectorAll('.LearnSkills_Page_displayPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDisplayPage.getAttribute('value');
            else
              item.textContent = inputDisplayPage.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_displayPage'));
        }

        if(inputClassCanonicalName) {
          document.querySelectorAll('.LearnSkills_Page_classCanonicalName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassCanonicalName.getAttribute('value');
            else
              item.textContent = inputClassCanonicalName.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_classCanonicalName'));
        }

        if(inputClassSimpleName) {
          document.querySelectorAll('.LearnSkills_Page_classSimpleName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassSimpleName.getAttribute('value');
            else
              item.textContent = inputClassSimpleName.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_classSimpleName'));
        }

        if(inputClassCanonicalNames) {
          document.querySelectorAll('.LearnSkills_Page_classCanonicalNames').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassCanonicalNames.getAttribute('value');
            else
              item.textContent = inputClassCanonicalNames.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_classCanonicalNames'));
        }

        if(inputSaves) {
          document.querySelectorAll('.LearnSkills_Page_saves').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSaves.getAttribute('value');
            else
              item.textContent = inputSaves.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_saves'));
        }

        if(inputObjectTitle) {
          document.querySelectorAll('.LearnSkills_Page_objectTitle').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectTitle.getAttribute('value');
            else
              item.textContent = inputObjectTitle.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_objectTitle'));
        }

        if(inputEditPage) {
          document.querySelectorAll('.LearnSkills_Page_editPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputEditPage.getAttribute('value');
            else
              item.textContent = inputEditPage.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_editPage'));
        }

        if(inputUserPage) {
          document.querySelectorAll('.LearnSkills_Page_userPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputUserPage.getAttribute('value');
            else
              item.textContent = inputUserPage.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_userPage'));
        }

        if(inputDownload) {
          document.querySelectorAll('.LearnSkills_Page_download').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDownload.getAttribute('value');
            else
              item.textContent = inputDownload.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_download'));
        }

        if(inputObjectSuggest) {
          document.querySelectorAll('.LearnSkills_Page_objectSuggest').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectSuggest.getAttribute('value');
            else
              item.textContent = inputObjectSuggest.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_objectSuggest'));
        }

        if(inputObjectText) {
          document.querySelectorAll('.LearnSkills_Page_objectText').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectText.getAttribute('value');
            else
              item.textContent = inputObjectText.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_objectText'));
        }

        if(inputSolrId) {
          document.querySelectorAll('.LearnSkills_Page_solrId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSolrId.getAttribute('value');
            else
              item.textContent = inputSolrId.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_solrId'));
        }

        if(inputCourseNum) {
          document.querySelectorAll('.LearnSkills_Page_courseNum').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputCourseNum.getAttribute('value');
            else
              item.textContent = inputCourseNum.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_courseNum'));
        }

        if(inputPageVideoUrl) {
          document.querySelectorAll('.LearnSkills_Page_pageVideoUrl').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageVideoUrl.getAttribute('value');
            else
              item.textContent = inputPageVideoUrl.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_pageVideoUrl'));
        }

        if(inputPageImageWidth) {
          document.querySelectorAll('.LearnSkills_Page_pageImageWidth').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageImageWidth.getAttribute('value');
            else
              item.textContent = inputPageImageWidth.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_pageImageWidth'));
        }

        if(inputPageImageHeight) {
          document.querySelectorAll('.LearnSkills_Page_pageImageHeight').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageImageHeight.getAttribute('value');
            else
              item.textContent = inputPageImageHeight.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_pageImageHeight'));
        }

        if(inputPageImageType) {
          document.querySelectorAll('.LearnSkills_Page_pageImageType').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageImageType.getAttribute('value');
            else
              item.textContent = inputPageImageType.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_pageImageType'));
        }

        if(inputPageImageAlt) {
          document.querySelectorAll('.LearnSkills_Page_pageImageAlt').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageImageAlt.getAttribute('value');
            else
              item.textContent = inputPageImageAlt.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_pageImageAlt'));
        }

        if(inputPrerequisiteArticleIds) {
          document.querySelectorAll('.LearnSkills_Page_prerequisiteArticleIds').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPrerequisiteArticleIds.getAttribute('value');
            else
              item.textContent = inputPrerequisiteArticleIds.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_prerequisiteArticleIds'));
        }

        if(inputPrerequisiteArticles) {
          document.querySelectorAll('.LearnSkills_Page_prerequisiteArticles').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPrerequisiteArticles.getAttribute('value');
            else
              item.textContent = inputPrerequisiteArticles.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_prerequisiteArticles'));
        }

        if(inputNextArticleIds) {
          document.querySelectorAll('.LearnSkills_Page_nextArticleIds').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputNextArticleIds.getAttribute('value');
            else
              item.textContent = inputNextArticleIds.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_nextArticleIds'));
        }

        if(inputNextArticles) {
          document.querySelectorAll('.LearnSkills_Page_nextArticles').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputNextArticles.getAttribute('value');
            else
              item.textContent = inputNextArticles.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_nextArticles'));
        }

        if(inputLabelsString) {
          document.querySelectorAll('.LearnSkills_Page_labelsString').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputLabelsString.getAttribute('value');
            else
              item.textContent = inputLabelsString.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_labelsString'));
        }

        if(inputLabels) {
          document.querySelectorAll('.LearnSkills_Page_labels').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputLabels.getAttribute('value');
            else
              item.textContent = inputLabels.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_labels'));
        }

        if(inputRelatedArticleIds) {
          document.querySelectorAll('.LearnSkills_Page_relatedArticleIds').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputRelatedArticleIds.getAttribute('value');
            else
              item.textContent = inputRelatedArticleIds.textContent;
          });
          addGlow(document.querySelector('.LearnSkills_Page_relatedArticleIds'));
        }

          pageGraphLearnSkills();
      });
    });
  }
}

function pageGraphLearnSkills(apiRequest) {
  ...
  if(r) {
    ...
    if(json['facetCounts']) {
      ...
      if(facetCounts['facetPivot'] && facetCounts['facetRanges']) {
        ...
        var layout = {};
        if(range) {
          layout['title'] = 'Learn cloud developer skills';
          layout['xaxis'] = {
          ...
          Plotly.react('htmBodyGraphLearnSkillsPage', data, layout);
        }
      }
    }
  }
...
function animateStats() {
  document.querySelector('#pageSearchVal-fqLearnSkills_time').innerText = '';
  searchPage('LearnSkills', function() {
    ...
    let animateInterval = window.setInterval(() => {
      ...
      document.querySelector('#fqLearnSkills_time').value = x;
      document.querySelector('#fqLearnSkills_time').onchange();
      searchPage('LearnSkills');
    }, speedRate);
  });
}
...
async function searchLearnSkills($formFilters, success, error) {
  var filters = searchLearnSkillsFilters($formFilters);
  if(success == null)
    success = function( data, textStatus, jQxhr ) {};
  if(error == null)
    error = function( jqXhr, target2 ) {};

  searchLearnSkillsVals(filters, target, success, error);
}

function searchLearnSkillsFilters($formFilters) {
  var filters = [];
  if($formFilters) {
...
function searchLearnSkillsVals(filters, target, success, error) {

  fetch(
    '/en-us/api/learn-skills?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
...
function suggestLearnSkillsObjectSuggest($formFilters, $list, target) {
  success = function( data, textStatus, jQxhr ) {
    if($list) {
      $list.innerHTML = '';
  ...
  error = function( jqXhr, target2 ) {};
  searchLearnSkillsVals($formFilters, target, success, error);
}

// GET //

async function getLearnSkills() {
  fetch(
    '/en-us/api/learn-skills/' + pageId
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
    }).then(response => {
      if(response.ok) {
...
async function patchLearnSkills($formFilters, $formValues, target, pageId, success, error) {
  var filters = patchLearnSkillsFilters($formFilters);
  ...
  patchLearnSkillsVals(pageId == null ? deparam(window.location.search ? window.location.search.substring(1) : window.location.search) : [{name:'fq', value:'pageId:' + pageId}], vals, target, success, error);
}

function patchLearnSkillsFilters($formFilters) {
  var filters = [];
  if($formFilters) {
...
function patchLearnSkillsVal(filters, v, val, target, success, error) {
  var vals = {};
  vals[v] = val;
  patchLearnSkillsVals(filters, vals, target, success, error);
}

function patchLearnSkillsVals(filters, vals, target, success, error) {
  fetch(
    '/en-us/api/learn-skills?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function postLearnSkills($formValues, target, success, error) {
  var vals = {};
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
...
function postLearnSkillsVals(vals, target, success, error) {
  fetch(
    '/en-us/api/learn-skills'
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function deleteLearnSkills(target, pageId, success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target, jqXhr);
      var url = data['editPage'];
  ...
  fetch(
    '/en-us/api/learn-skills/' + encodeURIComponent(pageId)
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'DELETE'
    }).then(response => {
...
async function putimportLearnSkills($formValues, target, pageId, success, error) {
  var json = $formValues.querySelector('.PUTImport_searchList')?.value;
  if(json != null && json !== '')
    putimportLearnSkillsVals(JSON.parse(json), target, success, error);
}

function putimportLearnSkillsVals(json, target, success, error) {
  fetch(
    '/en-us/api/learn-skills-import'
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'PUT'
...
async function deletefilterLearnSkills(target, success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target, jqXhr);
      var url = data['editPage'];
