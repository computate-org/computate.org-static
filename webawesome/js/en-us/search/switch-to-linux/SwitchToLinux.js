...
async function websocketSwitchToLinux(success) {
  window.eventBus.onopen = function () {

    window.eventBus.registerHandler('websocketSwitchToLinux', function (error, message) {
      ...
      iTemplate.innerHTML = '<i class="fa-brands fa-linux"></i>';
      var $i = iTemplate.content;
      var $headerSpan = document.createElement('span');
      $headerSpan.setAttribute('class', '');
      $headerSpan.innerText = 'modify Switch to Linux in ' + json.timeRemaining;
...
async function websocketSwitchToLinuxInner(apiRequest) {
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        if(vars.includes('created'))
          inputCreated = $response.querySelector('.SwitchToLinux_Page_created');
        if(vars.includes('modified'))
          inputModified = $response.querySelector('.SwitchToLinux_Page_modified');
        if(vars.includes('archived'))
          inputArchived = $response.querySelector('.SwitchToLinux_Page_archived');
        if(vars.includes('name'))
          inputName = $response.querySelector('.SwitchToLinux_Page_name');
        if(vars.includes('description'))
          inputDescription = $response.querySelector('.SwitchToLinux_Page_description');
        if(vars.includes('authorName'))
          inputAuthorName = $response.querySelector('.SwitchToLinux_Page_authorName');
        if(vars.includes('authorUrl'))
          inputAuthorUrl = $response.querySelector('.SwitchToLinux_Page_authorUrl');
        if(vars.includes('pageImageUri'))
          inputPageImageUri = $response.querySelector('.SwitchToLinux_Page_pageImageUri');
        if(vars.includes('pageId'))
          inputPageId = $response.querySelector('.SwitchToLinux_Page_pageId');
        if(vars.includes('displayPage'))
          inputDisplayPage = $response.querySelector('.SwitchToLinux_Page_displayPage');
        if(vars.includes('classCanonicalName'))
          inputClassCanonicalName = $response.querySelector('.SwitchToLinux_Page_classCanonicalName');
        if(vars.includes('classSimpleName'))
          inputClassSimpleName = $response.querySelector('.SwitchToLinux_Page_classSimpleName');
        if(vars.includes('classCanonicalNames'))
          inputClassCanonicalNames = $response.querySelector('.SwitchToLinux_Page_classCanonicalNames');
        if(vars.includes('saves'))
          inputSaves = $response.querySelector('.SwitchToLinux_Page_saves');
        if(vars.includes('objectTitle'))
          inputObjectTitle = $response.querySelector('.SwitchToLinux_Page_objectTitle');
        if(vars.includes('editPage'))
          inputEditPage = $response.querySelector('.SwitchToLinux_Page_editPage');
        if(vars.includes('userPage'))
          inputUserPage = $response.querySelector('.SwitchToLinux_Page_userPage');
        if(vars.includes('download'))
          inputDownload = $response.querySelector('.SwitchToLinux_Page_download');
        if(vars.includes('objectSuggest'))
          inputObjectSuggest = $response.querySelector('.SwitchToLinux_Page_objectSuggest');
        if(vars.includes('objectText'))
          inputObjectText = $response.querySelector('.SwitchToLinux_Page_objectText');
        if(vars.includes('solrId'))
          inputSolrId = $response.querySelector('.SwitchToLinux_Page_solrId');
        if(vars.includes('courseNum'))
          inputCourseNum = $response.querySelector('.SwitchToLinux_Page_courseNum');
        if(vars.includes('pageVideoUrl'))
          inputPageVideoUrl = $response.querySelector('.SwitchToLinux_Page_pageVideoUrl');
        if(vars.includes('pageImageWidth'))
          inputPageImageWidth = $response.querySelector('.SwitchToLinux_Page_pageImageWidth');
        if(vars.includes('pageImageHeight'))
          inputPageImageHeight = $response.querySelector('.SwitchToLinux_Page_pageImageHeight');
        if(vars.includes('pageImageType'))
          inputPageImageType = $response.querySelector('.SwitchToLinux_Page_pageImageType');
        if(vars.includes('pageImageAlt'))
          inputPageImageAlt = $response.querySelector('.SwitchToLinux_Page_pageImageAlt');
        if(vars.includes('prerequisiteArticleIds'))
          inputPrerequisiteArticleIds = $response.querySelector('.SwitchToLinux_Page_prerequisiteArticleIds');
        if(vars.includes('prerequisiteArticles'))
          inputPrerequisiteArticles = $response.querySelector('.SwitchToLinux_Page_prerequisiteArticles');
        if(vars.includes('nextArticleIds'))
          inputNextArticleIds = $response.querySelector('.SwitchToLinux_Page_nextArticleIds');
        if(vars.includes('nextArticles'))
          inputNextArticles = $response.querySelector('.SwitchToLinux_Page_nextArticles');
        if(vars.includes('labelsString'))
          inputLabelsString = $response.querySelector('.SwitchToLinux_Page_labelsString');
        if(vars.includes('labels'))
          inputLabels = $response.querySelector('.SwitchToLinux_Page_labels');
        if(vars.includes('relatedArticleIds'))
          inputRelatedArticleIds = $response.querySelector('.SwitchToLinux_Page_relatedArticleIds');

        jsWebsocketSwitchToLinux(pageId, vars, $response);
        window.result = JSON.parse($response.querySelector('.pageForm .result')?.value);
        window.listSwitchToLinux = JSON.parse($response.querySelector('.pageForm .listSwitchToLinux')?.value);


        if(inputCreated) {
          document.querySelectorAll('.SwitchToLinux_Page_created').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputCreated.getAttribute('value');
            else
              item.textContent = inputCreated.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_created'));
        }

        if(inputModified) {
          document.querySelectorAll('.SwitchToLinux_Page_modified').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputModified.getAttribute('value');
            else
              item.textContent = inputModified.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_modified'));
        }

        if(inputArchived) {
          document.querySelectorAll('.SwitchToLinux_Page_archived').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputArchived.getAttribute('value');
            else
              item.textContent = inputArchived.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_archived'));
        }

        if(inputName) {
          document.querySelectorAll('.SwitchToLinux_Page_name').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputName.getAttribute('value');
            else
              item.textContent = inputName.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_name'));
        }

        if(inputDescription) {
          document.querySelectorAll('.SwitchToLinux_Page_description').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDescription.getAttribute('value');
            else
              item.textContent = inputDescription.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_description'));
        }

        if(inputAuthorName) {
          document.querySelectorAll('.SwitchToLinux_Page_authorName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAuthorName.getAttribute('value');
            else
              item.textContent = inputAuthorName.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_authorName'));
        }

        if(inputAuthorUrl) {
          document.querySelectorAll('.SwitchToLinux_Page_authorUrl').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAuthorUrl.getAttribute('value');
            else
              item.textContent = inputAuthorUrl.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_authorUrl'));
        }

        if(inputPageImageUri) {
          document.querySelectorAll('.SwitchToLinux_Page_pageImageUri').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageImageUri.getAttribute('value');
            else
              item.textContent = inputPageImageUri.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_pageImageUri'));
        }

        if(inputPageId) {
          document.querySelectorAll('.SwitchToLinux_Page_pageId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageId.getAttribute('value');
            else
              item.textContent = inputPageId.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_pageId'));
        }

        if(inputDisplayPage) {
          document.querySelectorAll('.SwitchToLinux_Page_displayPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDisplayPage.getAttribute('value');
            else
              item.textContent = inputDisplayPage.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_displayPage'));
        }

        if(inputClassCanonicalName) {
          document.querySelectorAll('.SwitchToLinux_Page_classCanonicalName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassCanonicalName.getAttribute('value');
            else
              item.textContent = inputClassCanonicalName.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_classCanonicalName'));
        }

        if(inputClassSimpleName) {
          document.querySelectorAll('.SwitchToLinux_Page_classSimpleName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassSimpleName.getAttribute('value');
            else
              item.textContent = inputClassSimpleName.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_classSimpleName'));
        }

        if(inputClassCanonicalNames) {
          document.querySelectorAll('.SwitchToLinux_Page_classCanonicalNames').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassCanonicalNames.getAttribute('value');
            else
              item.textContent = inputClassCanonicalNames.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_classCanonicalNames'));
        }

        if(inputSaves) {
          document.querySelectorAll('.SwitchToLinux_Page_saves').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSaves.getAttribute('value');
            else
              item.textContent = inputSaves.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_saves'));
        }

        if(inputObjectTitle) {
          document.querySelectorAll('.SwitchToLinux_Page_objectTitle').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectTitle.getAttribute('value');
            else
              item.textContent = inputObjectTitle.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_objectTitle'));
        }

        if(inputEditPage) {
          document.querySelectorAll('.SwitchToLinux_Page_editPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputEditPage.getAttribute('value');
            else
              item.textContent = inputEditPage.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_editPage'));
        }

        if(inputUserPage) {
          document.querySelectorAll('.SwitchToLinux_Page_userPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputUserPage.getAttribute('value');
            else
              item.textContent = inputUserPage.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_userPage'));
        }

        if(inputDownload) {
          document.querySelectorAll('.SwitchToLinux_Page_download').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDownload.getAttribute('value');
            else
              item.textContent = inputDownload.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_download'));
        }

        if(inputObjectSuggest) {
          document.querySelectorAll('.SwitchToLinux_Page_objectSuggest').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectSuggest.getAttribute('value');
            else
              item.textContent = inputObjectSuggest.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_objectSuggest'));
        }

        if(inputObjectText) {
          document.querySelectorAll('.SwitchToLinux_Page_objectText').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectText.getAttribute('value');
            else
              item.textContent = inputObjectText.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_objectText'));
        }

        if(inputSolrId) {
          document.querySelectorAll('.SwitchToLinux_Page_solrId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSolrId.getAttribute('value');
            else
              item.textContent = inputSolrId.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_solrId'));
        }

        if(inputCourseNum) {
          document.querySelectorAll('.SwitchToLinux_Page_courseNum').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputCourseNum.getAttribute('value');
            else
              item.textContent = inputCourseNum.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_courseNum'));
        }

        if(inputPageVideoUrl) {
          document.querySelectorAll('.SwitchToLinux_Page_pageVideoUrl').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageVideoUrl.getAttribute('value');
            else
              item.textContent = inputPageVideoUrl.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_pageVideoUrl'));
        }

        if(inputPageImageWidth) {
          document.querySelectorAll('.SwitchToLinux_Page_pageImageWidth').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageImageWidth.getAttribute('value');
            else
              item.textContent = inputPageImageWidth.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_pageImageWidth'));
        }

        if(inputPageImageHeight) {
          document.querySelectorAll('.SwitchToLinux_Page_pageImageHeight').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageImageHeight.getAttribute('value');
            else
              item.textContent = inputPageImageHeight.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_pageImageHeight'));
        }

        if(inputPageImageType) {
          document.querySelectorAll('.SwitchToLinux_Page_pageImageType').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageImageType.getAttribute('value');
            else
              item.textContent = inputPageImageType.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_pageImageType'));
        }

        if(inputPageImageAlt) {
          document.querySelectorAll('.SwitchToLinux_Page_pageImageAlt').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageImageAlt.getAttribute('value');
            else
              item.textContent = inputPageImageAlt.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_pageImageAlt'));
        }

        if(inputPrerequisiteArticleIds) {
          document.querySelectorAll('.SwitchToLinux_Page_prerequisiteArticleIds').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPrerequisiteArticleIds.getAttribute('value');
            else
              item.textContent = inputPrerequisiteArticleIds.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_prerequisiteArticleIds'));
        }

        if(inputPrerequisiteArticles) {
          document.querySelectorAll('.SwitchToLinux_Page_prerequisiteArticles').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPrerequisiteArticles.getAttribute('value');
            else
              item.textContent = inputPrerequisiteArticles.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_prerequisiteArticles'));
        }

        if(inputNextArticleIds) {
          document.querySelectorAll('.SwitchToLinux_Page_nextArticleIds').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputNextArticleIds.getAttribute('value');
            else
              item.textContent = inputNextArticleIds.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_nextArticleIds'));
        }

        if(inputNextArticles) {
          document.querySelectorAll('.SwitchToLinux_Page_nextArticles').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputNextArticles.getAttribute('value');
            else
              item.textContent = inputNextArticles.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_nextArticles'));
        }

        if(inputLabelsString) {
          document.querySelectorAll('.SwitchToLinux_Page_labelsString').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputLabelsString.getAttribute('value');
            else
              item.textContent = inputLabelsString.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_labelsString'));
        }

        if(inputLabels) {
          document.querySelectorAll('.SwitchToLinux_Page_labels').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputLabels.getAttribute('value');
            else
              item.textContent = inputLabels.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_labels'));
        }

        if(inputRelatedArticleIds) {
          document.querySelectorAll('.SwitchToLinux_Page_relatedArticleIds').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputRelatedArticleIds.getAttribute('value');
            else
              item.textContent = inputRelatedArticleIds.textContent;
          });
          addGlow(document.querySelector('.SwitchToLinux_Page_relatedArticleIds'));
        }

          pageGraphSwitchToLinux();
      });
    });
  }
}

function pageGraphSwitchToLinux(apiRequest) {
    ...
    if(json['facetCounts']) {
      ...
      if(facetCounts['facetPivot'] && facetCounts['facetRanges']) {
        ...
        if(range) {
          ...
          Plotly.react('htmBodyGraphSwitchToLinuxPage', data, layout);
        }
      }
    }
  }
...
function animateStats() {
  document.querySelector('#pageSearchVal-fqSwitchToLinux_time').innerText = '';
  searchPage('SwitchToLinux', function() {
    ...
    let animateInterval = window.setInterval(() => {
      ...
      document.querySelector('#fqSwitchToLinux_time').value = x;
      document.querySelector('#fqSwitchToLinux_time').onchange();
      searchPage('SwitchToLinux');
    }, speedRate);
  });
}
...
async function searchSwitchToLinux($formFilters, success, error) {
  var filters = searchSwitchToLinuxFilters($formFilters);
  if(success == null)
    success = function( data, textStatus, jQxhr ) {};
  if(error == null)
    error = function( jqXhr, target2 ) {};

  searchSwitchToLinuxVals(filters, target, success, error);
}

function searchSwitchToLinuxFilters($formFilters) {
  var filters = [];
  if($formFilters) {
...
function searchSwitchToLinuxVals(filters, target, success, error) {

  fetch(
    '/en-us/api/switch-to-linux?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
...
function suggestSwitchToLinuxObjectSuggest($formFilters, $list, target) {
  success = function( data, textStatus, jQxhr ) {
    if($list) {
      $list.innerHTML = '';
      data['list'].forEach((o, i) => {
        var $i = document.querySelector('<i class="fa-brands fa-linux"></i>');
  ...
  error = function( jqXhr, target2 ) {};
  searchSwitchToLinuxVals($formFilters, target, success, error);
}

// GET //

async function getSwitchToLinux() {
  fetch(
    '/en-us/api/switch-to-linux/' + pageId
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
    }).then(response => {
      if(response.ok) {
...
async function patchSwitchToLinux($formFilters, $formValues, target, pageId, success, error) {
  var filters = patchSwitchToLinuxFilters($formFilters);
  ...
  patchSwitchToLinuxVals(pageId == null ? deparam(window.location.search ? window.location.search.substring(1) : window.location.search) : [{name:'fq', value:'pageId:' + pageId}], vals, target, success, error);
}

function patchSwitchToLinuxFilters($formFilters) {
  var filters = [];
  if($formFilters) {
...
function patchSwitchToLinuxVal(filters, v, val, target, success, error) {
  var vals = {};
  vals[v] = val;
  patchSwitchToLinuxVals(filters, vals, target, success, error);
}

function patchSwitchToLinuxVals(filters, vals, target, success, error) {
  fetch(
    '/en-us/api/switch-to-linux?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function postSwitchToLinux($formValues, target, success, error) {
  var vals = {};
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
...
function postSwitchToLinuxVals(vals, target, success, error) {
  fetch(
    '/en-us/api/switch-to-linux'
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function deleteSwitchToLinux(target, pageId, success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target, jqXhr);
      var url = data['editPage'];
  ...
  fetch(
    '/en-us/api/switch-to-linux/' + encodeURIComponent(pageId)
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'DELETE'
    }).then(response => {
...
async function putimportSwitchToLinux($formValues, target, pageId, success, error) {
  var json = $formValues.querySelector('.PUTImport_searchList')?.value;
  if(json != null && json !== '')
    putimportSwitchToLinuxVals(JSON.parse(json), target, success, error);
}

function putimportSwitchToLinuxVals(json, target, success, error) {
  fetch(
    '/en-us/api/switch-to-linux-import'
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'PUT'
...
async function deletefilterSwitchToLinux(target, success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target, jqXhr);
      var url = data['editPage'];
