...
async function websocketDeploySpine(success) {
  window.eventBus.onopen = function () {

    window.eventBus.registerHandler('websocketDeploySpine', function (error, message) {
      ...
      iTemplate.innerHTML = '<i class="fa-kit fa-openshift"></i>';
...
async function websocketDeploySpineInner(apiRequest) {
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        if(vars.includes('created'))
          inputCreated = $response.querySelector('.DeploySpine_Page_created');
        if(vars.includes('modified'))
          inputModified = $response.querySelector('.DeploySpine_Page_modified');
        if(vars.includes('archived'))
          inputArchived = $response.querySelector('.DeploySpine_Page_archived');
        ...
        if(vars.includes('pageId'))
          inputPageId = $response.querySelector('.DeploySpine_Page_pageId');
        if(vars.includes('displayPage'))
          inputDisplayPage = $response.querySelector('.DeploySpine_Page_displayPage');
        if(vars.includes('classCanonicalName'))
          inputClassCanonicalName = $response.querySelector('.DeploySpine_Page_classCanonicalName');
        if(vars.includes('classSimpleName'))
          inputClassSimpleName = $response.querySelector('.DeploySpine_Page_classSimpleName');
        if(vars.includes('classCanonicalNames'))
          inputClassCanonicalNames = $response.querySelector('.DeploySpine_Page_classCanonicalNames');
        if(vars.includes('saves'))
          inputSaves = $response.querySelector('.DeploySpine_Page_saves');
        if(vars.includes('objectTitle'))
          inputObjectTitle = $response.querySelector('.DeploySpine_Page_objectTitle');
        if(vars.includes('editPage'))
          inputEditPage = $response.querySelector('.DeploySpine_Page_editPage');
        if(vars.includes('userPage'))
          inputUserPage = $response.querySelector('.DeploySpine_Page_userPage');
        if(vars.includes('download'))
          inputDownload = $response.querySelector('.DeploySpine_Page_download');
        if(vars.includes('objectSuggest'))
          inputObjectSuggest = $response.querySelector('.DeploySpine_Page_objectSuggest');
        if(vars.includes('objectText'))
          inputObjectText = $response.querySelector('.DeploySpine_Page_objectText');
        if(vars.includes('solrId'))
          inputSolrId = $response.querySelector('.DeploySpine_Page_solrId');
        if(vars.includes('courseNum'))
          inputCourseNum = $response.querySelector('.DeploySpine_Page_courseNum');
        if(vars.includes('pageVideoUrl'))
          inputPageVideoUrl = $response.querySelector('.DeploySpine_Page_pageVideoUrl');
        if(vars.includes('pageImageWidth'))
          inputPageImageWidth = $response.querySelector('.DeploySpine_Page_pageImageWidth');
        if(vars.includes('pageImageHeight'))
          inputPageImageHeight = $response.querySelector('.DeploySpine_Page_pageImageHeight');
        ...
        jsWebsocketDeploySpine(pageId, vars, $response);
        window.result = JSON.parse($response.querySelector('.pageForm .result')?.value);
        window.listDeploySpine = JSON.parse($response.querySelector('.pageForm .listDeploySpine')?.value);


        if(inputCreated) {
          document.querySelectorAll('.DeploySpine_Page_created').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputCreated.getAttribute('value');
            else
              item.textContent = inputCreated.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_created'));
        }

        if(inputModified) {
          document.querySelectorAll('.DeploySpine_Page_modified').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputModified.getAttribute('value');
            else
              item.textContent = inputModified.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_modified'));
        }

        if(inputArchived) {
          document.querySelectorAll('.DeploySpine_Page_archived').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputArchived.getAttribute('value');
            else
              item.textContent = inputArchived.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_archived'));
        }

        if(inputName) {
          document.querySelectorAll('.DeploySpine_Page_name').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputName.getAttribute('value');
            else
              item.textContent = inputName.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_name'));
        }

        if(inputDescription) {
          document.querySelectorAll('.DeploySpine_Page_description').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
        ...
        if(inputPageImageUri) {
          document.querySelectorAll('.DeploySpine_Page_pageImageUri').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageImageUri.getAttribute('value');
            else
              item.textContent = inputPageImageUri.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_pageImageUri'));
        }

        if(inputPageId) {
          document.querySelectorAll('.DeploySpine_Page_pageId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageId.getAttribute('value');
            else
              item.textContent = inputPageId.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_pageId'));
        }

        if(inputDisplayPage) {
          document.querySelectorAll('.DeploySpine_Page_displayPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDisplayPage.getAttribute('value');
            else
              item.textContent = inputDisplayPage.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_displayPage'));
        }

        if(inputClassCanonicalName) {
          document.querySelectorAll('.DeploySpine_Page_classCanonicalName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassCanonicalName.getAttribute('value');
            else
              item.textContent = inputClassCanonicalName.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_classCanonicalName'));
        }

        if(inputClassSimpleName) {
          document.querySelectorAll('.DeploySpine_Page_classSimpleName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassSimpleName.getAttribute('value');
            else
              item.textContent = inputClassSimpleName.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_classSimpleName'));
        }

        if(inputClassCanonicalNames) {
          document.querySelectorAll('.DeploySpine_Page_classCanonicalNames').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassCanonicalNames.getAttribute('value');
            else
              item.textContent = inputClassCanonicalNames.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_classCanonicalNames'));
        }

        if(inputSaves) {
          document.querySelectorAll('.DeploySpine_Page_saves').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSaves.getAttribute('value');
            else
              item.textContent = inputSaves.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_saves'));
        }

        if(inputObjectTitle) {
          document.querySelectorAll('.DeploySpine_Page_objectTitle').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectTitle.getAttribute('value');
            else
              item.textContent = inputObjectTitle.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_objectTitle'));
        }

        if(inputEditPage) {
          document.querySelectorAll('.DeploySpine_Page_editPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputEditPage.getAttribute('value');
            else
              item.textContent = inputEditPage.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_editPage'));
        }

        if(inputUserPage) {
          document.querySelectorAll('.DeploySpine_Page_userPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputUserPage.getAttribute('value');
            else
              item.textContent = inputUserPage.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_userPage'));
        }

        if(inputDownload) {
          document.querySelectorAll('.DeploySpine_Page_download').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDownload.getAttribute('value');
            else
              item.textContent = inputDownload.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_download'));
        }

        if(inputObjectSuggest) {
          document.querySelectorAll('.DeploySpine_Page_objectSuggest').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectSuggest.getAttribute('value');
            else
              item.textContent = inputObjectSuggest.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_objectSuggest'));
        }

        if(inputObjectText) {
          document.querySelectorAll('.DeploySpine_Page_objectText').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectText.getAttribute('value');
            else
              item.textContent = inputObjectText.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_objectText'));
        }

        if(inputSolrId) {
          document.querySelectorAll('.DeploySpine_Page_solrId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSolrId.getAttribute('value');
            else
              item.textContent = inputSolrId.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_solrId'));
        }

        if(inputCourseNum) {
          document.querySelectorAll('.DeploySpine_Page_courseNum').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputCourseNum.getAttribute('value');
            else
              item.textContent = inputCourseNum.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_courseNum'));
        }

        if(inputPageVideoUrl) {
          document.querySelectorAll('.DeploySpine_Page_pageVideoUrl').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageVideoUrl.getAttribute('value');
            else
              item.textContent = inputPageVideoUrl.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_pageVideoUrl'));
        }

        if(inputPageImageWidth) {
          document.querySelectorAll('.DeploySpine_Page_pageImageWidth').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageImageWidth.getAttribute('value');
            else
              item.textContent = inputPageImageWidth.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_pageImageWidth'));
        }

        if(inputPageImageHeight) {
          document.querySelectorAll('.DeploySpine_Page_pageImageHeight').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageImageHeight.getAttribute('value');
            else
              item.textContent = inputPageImageHeight.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_pageImageHeight'));
        }

        if(inputPageImageType) {
          document.querySelectorAll('.DeploySpine_Page_pageImageType').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageImageType.getAttribute('value');
            else
              item.textContent = inputPageImageType.textContent;
          });
          addGlow(document.querySelector('.DeploySpine_Page_pageImageType'));
        }

        if(inputPageImageAlt) {
          document.querySelectorAll('.DeploySpine_Page_pageImageAlt').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
        ...
        if(inputRelatedArticleIds) {
          ...
          addGlow(document.querySelector('.DeploySpine_Page_relatedArticleIds'));
        }

          pageGraphDeploySpine();
      });
    });
  }
}

function pageGraphDeploySpine(apiRequest) {
    ...
    if(json['facetCounts']) {
      ...
      if(facetCounts['facetPivot'] && facetCounts['facetRanges']) {
        ...
        if(range) {
          ...
          Plotly.react('htmBodyGraphDeploySpinePage', data, layout);
        }
      }
    }
  }
...
function animateStats() {
  document.querySelector('#pageSearchVal-fqDeploySpine_time').innerText = '';
  searchPage('DeploySpine', function() {
    ...
    let animateInterval = window.setInterval(() => {
      ...
      document.querySelector('#fqDeploySpine_time').value = x;
      document.querySelector('#fqDeploySpine_time').onchange();
      searchPage('DeploySpine');
    }, speedRate);
  });
}
...
async function searchDeploySpine($formFilters, success, error) {
  var filters = searchDeploySpineFilters($formFilters);
  if(success == null)
    success = function( data, textStatus, jQxhr ) {};
  if(error == null)
    error = function( jqXhr, target2 ) {};

  searchDeploySpineVals(filters, target, success, error);
}

function searchDeploySpineFilters($formFilters) {
  var filters = [];
  if($formFilters) {
...
function searchDeploySpineVals(filters, target, success, error) {

  fetch(
    '/en-us/api/deploy-spine?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
...
function suggestDeploySpineObjectSuggest($formFilters, $list, target) {
  success = function( data, textStatus, jQxhr ) {
    if($list) {
      $list.innerHTML = '';
      data['list'].forEach((o, i) => {
        var $i = document.querySelector('<i class="fa-kit fa-openshift"></i>');
  ...
  error = function( jqXhr, target2 ) {};
  searchDeploySpineVals($formFilters, target, success, error);
}

// GET //

async function getDeploySpine() {
  fetch(
    '/en-us/api/deploy-spine/' + pageId
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
    }).then(response => {
      if(response.ok) {
...
async function patchDeploySpine($formFilters, $formValues, target, pageId, success, error) {
  var filters = patchDeploySpineFilters($formFilters);
  ...
  patchDeploySpineVals(pageId == null ? deparam(window.location.search ? window.location.search.substring(1) : window.location.search) : [{name:'fq', value:'pageId:' + pageId}], vals, target, success, error);
}

function patchDeploySpineFilters($formFilters) {
  var filters = [];
  if($formFilters) {
...
function patchDeploySpineVal(filters, v, val, target, success, error) {
  var vals = {};
  vals[v] = val;
  patchDeploySpineVals(filters, vals, target, success, error);
}

function patchDeploySpineVals(filters, vals, target, success, error) {
  fetch(
    '/en-us/api/deploy-spine?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function postDeploySpine($formValues, target, success, error) {
  var vals = {};
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
...
function postDeploySpineVals(vals, target, success, error) {
  fetch(
    '/en-us/api/deploy-spine'
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function deleteDeploySpine(target, pageId, success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target, jqXhr);
      var url = data['editPage'];
  ...
  fetch(
    '/en-us/api/deploy-spine/' + encodeURIComponent(pageId)
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'DELETE'
    }).then(response => {
...
async function putimportDeploySpine($formValues, target, pageId, success, error) {
  var json = $formValues.querySelector('.PUTImport_searchList')?.value;
  if(json != null && json !== '')
    putimportDeploySpineVals(JSON.parse(json), target, success, error);
}

function putimportDeploySpineVals(json, target, success, error) {
  fetch(
    '/en-us/api/deploy-spine-import'
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'PUT'
...
async function deletefilterDeploySpine(target, success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target, jqXhr);
      var url = data['editPage'];
