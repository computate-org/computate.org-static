    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        if(vars.includes('pageVideoUrl'))
          inputPageVideoUrl = $response.querySelector('.CompanyProduct_Page_pageVideoUrl');
        ...
        if(inputPageVideoUrl) {
          document.querySelectorAll('.CompanyProduct_Page_pageVideoUrl').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPageVideoUrl.getAttribute('value');
            else
              item.textContent = inputPageVideoUrl.textContent;
          });
          addGlow(document.querySelector('.CompanyProduct_Page_pageVideoUrl'));
        }
...
function searchCompanyProductVals(filters, target, success, error) {
  ...
  fetch(
    '/en-us/api/solution?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
...
function patchpayCompanyProductVals(filters, vals, target, success, error) {
  fetch(
    '/en-us/shop/solution/{pageId}?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'PATCH'
      , body: JSON.stringify(vals)
...
async function deleteCompanyProduct(target, pageId, success, error) {
  ...
  fetch(
    '/en-us/api/solution/' + encodeURIComponent(pageId)
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'DELETE'
    }).then(response => {
