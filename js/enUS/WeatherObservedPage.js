...
function suggestWeatherObservedObjectSuggest($formFilters, $list, target) {
  success = function( data, textStatus, jQxhr ) {
    ...
    data['list'].forEach((o, i) => {
      var $i = document.querySelector('<i>').setAttribute('class', 'fas fa-cloud-bolt-sun ');
...
async function websocketWeatherObserved(success) {
  window.eventBus.onopen = function () {
    ...
    window.eventBus.registerHandler('websocketWeatherObserved', function (error, message) {
      ...
      var id = json['id'];
      var pk = json['pk'];
      var pkPage = document.querySelector('#WeatherObservedForm :input[name=pk]')?.value;
      var pks = json['pks'];
      ...
      var $i = document.querySelector('<i>').setAttribute('class', 'fas fa-cloud-bolt-sun w3-margin-right ').setAttribute('id', 'icon-' + id);
      var $headerSpan = document.querySelector('<span>').setAttribute('class', '').text('modify weather observed devices in ' + json.timeRemaining);
...
function animateStats() {
  document.querySelector('#pageSearchVal-fqWeatherObserved_time').text('');
