...
async function websocketWeatherObserved(success) {
  window.eventBus.onopen = function () {
    ...
    window.eventBus.registerHandler('websocketWeatherObserved', function (error, message) {
      ...
      var $progress = document.createElement('<div>').setAttribute('class', 'w3-2017-shaded-spruce ').setAttribute('style', 'height: 24px; width: ' + percent + '; ').setAttribute('id', 'progress-' + id).text(numPATCH + '/' + numFound);
      $card.append($header);
      $header.append($i);
