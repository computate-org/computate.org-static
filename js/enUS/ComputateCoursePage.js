...
async function searchComputateCourse($formFilters, success, error) {
  var filters = searchComputateCourseFilters($formFilters);
  if(success == null)
    success = function( data, textStatus, jQxhr ) {};
  if(error == null)
    error = function( jqXhr, textStatus, errorThrown ) {};

  searchComputateCourseVals(filters, target, success, error);
}

function searchComputateCourseFilters($formFilters) {
  var filters = [];
  if($formFilters) {
...
function searchComputateCourseVals(filters, target, success, error) {
...
function suggestComputateCourseObjectSuggest($formFilters, $list, target) {
  success = function( data, textStatus, jQxhr ) {
    $list.empty();
  ...
  error = function( jqXhr, textStatus, errorThrown ) {};
  searchComputateCourseVals($formFilters, target, success, error);
}

// GET //

async function getComputateCourse(pk) {
  fetch(
    '/api/course/' + id
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function patchComputateCourse($formFilters, $formValues, pk, success, error) {
  var filters = patchComputateCourseFilters($formFilters);
  ...
  patchComputateCourseVals(pk == null ? $.deparam(window.location.search ? window.location.search.substring(1) : window.location.search) : [{name:'fq', value:'pk:' + pk}], vals, target, success, error);
}

function patchComputateCourseFilters($formFilters) {
  var filters = [];
  if($formFilters) {
...
function patchComputateCourseVal(filters, v, val, target, success, error) {
  var vals = {};
  vals[v] = val;
  patchComputateCourseVals(filters, vals, target, success, error);
}

function patchComputateCourseVals(filters, vals, target, success, error) {
  fetch(
    '/api/course?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function postComputateCourse($formValues, target, success, error) {
  var vals = {};
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
...
function postComputateCourseVals(vals, target, success, error) {
  fetch(
    '/api/course'
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function putimportComputateCourse($formValues, pk, success, error) {
  var json = $formValues.querySelector('.PUTImport_searchList')?.value;
  if(json != null && json !== '')
    putimportComputateCourseVals(JSON.parse(json), target, success, error);
}

function putimportComputateCourseVals(json, target, success, error) {
  fetch(
    '/api/course-import'
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function websocketComputateCourse(success) {
  window.eventBus.onopen = function () {

    window.eventBus.registerHandler('websocketComputateCourse', function (error, message) {
      var json = JSON.parse(message['body']);
      var id = json['id'];
      var pk = json['pk'];
      var pkPage = document.querySelector('#ComputateCourseForm :input[name=pk]')?.value;
      var pks = json['pks'];
...
async function websocketComputateCourseInner(apiRequest) {
  ...
  if(pk != null && vars.length > 0) {
    ...
    $.get(uri, {}, function(data) {
      var $response = $("<html/>").html(data);
        ...
        jsWebsocketComputateCourse(pk, vars, $response);

        window.computateCourse = JSON.parse($response.querySelector('.pageForm .computateCourse')?.value);
        window.listComputateCourse = JSON.parse($response.querySelector('.pageForm .listComputateCourse')?.value);
        ...
        pageGraphComputateCourse();
    });
  }
}

function pageGraphComputateCourse(apiRequest) {
...
function animateStats() {
  document.querySelector('#pageSearchVal-fqComputateCourse_time').text('');
  searchPage('ComputateCourse', function() {
    ...
    let animateInterval = window.setInterval(() => {
      ...
      document.querySelector('#fqComputateCourse_time').value = x;
      document.querySelector('#fqComputateCourse_time').onchange();
      searchPage('ComputateCourse');
    }, speedRate);
  });
}
