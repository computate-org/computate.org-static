...
async function patchCompanyResearch($formFilters, $formValues, id, success, error) {
...
async function putimportCompanyResearch($formValues, id, success, error) {
...
async function websocketCompanyResearch(success) {
  window.eventBus.onopen = function () {
    ...
    window.eventBus.registerHandler('websocketCompanyResearch', function (error, message) {
      ...
      var id = json['id'];
      var pk = json['pk'];
      var pkPage = document.querySelector('#CompanyResearchForm :input[name=id]')?.value;
      var pks = json['pks'];
      ...
      var $i = document.createElement('<i class="fa-duotone fa-solid fa-school"></i>');
      var $headerSpan = document.createElement('<span>').setAttribute('class', '').text('modify research in ' + json.timeRemaining);
